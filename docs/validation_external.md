...
가능한 설명은 셋이고 아직 가르지 못했다 — (a) 우리 NCC EF 0.95가 낮다, (b) 우리 크래커 능력 0.455 Mt가 낮다(에틸렌 기준 vs 총 처리량), (c) 공시 감축량이 크래커 밖(공장 유틸리티·다운스트림 가열로)을 포함한다. **어느 쪽이든 석유화학 배출 입력이 검증되지 않았다는 §5의 판정을 강화한다.** (a)라면 우리는 석화 감축량을 과소평가하고 **감축 단가 ②(237~275)를 과대평가**하고 있다 — 값을 고치지 않고 이 사실을 기록만 하는 이유는 (b)·(c)를 배제할 자료가 아직 없기 때문이다.
