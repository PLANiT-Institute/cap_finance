...
| A-02 | 시설 배출 = 회사 실측을 능력×루트EF 가중 배분 | 시설별 실측 미확보 | **최대** (민감도 1위, T5) | **H2: POSCO ±2.7% 통과 / NSC +15.7% 초과** — `validation_backtest.md` |
| A-03 | 에너지·배출 원단위 = 루트 표준값 주입 (BF 2.15 tCO₂/t 등, **범위 없음**) | 기업 공시 부재 | **대** — NSC 편차의 원인으로 특정됨 | H2 가설 배제표 |
...
1. **NSC의 배출 원단위가 실적보다 15.7% 높다**(H2 후향 검증). 원인은 설비 목록이 아니다 — 가동 중인 전기로를 전부 넣어도 1.3%p만 닫히고 FY2029 신설분까지 넣어도 +10.2%가 남는다(H2 §1의 가설 배제표). **루트 표준값 `BF = 2.15 tCO₂/t`(범위 없는 T5 주입값) 자체가 NSC에 대해 높다.** 모형이 기존 설비를 실제보다 더 더럽다고 보므로 **전환 감축량을 과대, 감축 단가 ②를 과소평가**한다. POSCO는 ±2.7%로 통과. 석유화학 2사는 생산량 미공시라 **대조 자체가 불가**하다.
