...
1. **철강의 배출 원단위는 검증되지만 석유화학의 것은 검증되지 않는다.** 철강은 시설 배출을 회사 공시 총량에 재척도하므로 모형이 쓰는 `ef_inc`가 공시와 ±1% 안에서 맞는다(NSC 1.846 vs 1.856, POSCO 1.987 vs 1.988). 루트 표준값(BF 2.15)은 **총량을 시설에 나누는 가중치**로만 쓰이고, NSC처럼 목록이 고로 단일 구성이면 가중치가 균일해져 표준값 수준이 결과에 걸리지 않는다. 반면 **석유화학은 재척도하지 않는 상향식**이라 주입값 0.95가 곧 수준이고, 두 회사 모두 생산량을 공시하지 않아 **확인할 방법이 없다**. 여기가 배출 입력의 진짜 빈 곳이다. 미쓰이 오사카 공시를 절대량으로 역산하면 우리 NCC EF
   0.95가 **1.3~1.8배 낮을 수 있다**(H3 §4-1) — 사실이라면 석화 감축량을 과소평가하고
   **② 237~275를 과대평가**하는 방향이다. 능력 기준(에틸렌 vs 총 처리량)과 공시 감축량의
   경계를 가르지 못해 값은 고치지 않고 기록만 했다.
2. **A-02가 최대 약점.** 감축 단가를 86% 움직이는 파라미터가 가장 약한 증거(T5) 위에 있다.
   시설별 실측 배출(GIR 명세서·SHK) 확보 전까지 시설 단위 절대값은 **순서 정보로만** 다뤄야 한다.
3. **TCaR 절대값은 사전 변동성에 의존한다** (A-17). 월별 이력 확보 전까지 TCaR의 *순위*는
   방어 가능하나 *수준*은 아니다.
...
6. **공시 해상도** — 4개 기업×시나리오 중 일부는 강제 가능한 커밋이 없어 gap을 산출하지
   않는다. 이것은 결측이 아니라 **측정 불가 판정**이다 (A-16).
7. **계획 선택 채널의 민감도는 부분적으로만 검증됐다.** 시나리오 러너는 계획 메뉴를
