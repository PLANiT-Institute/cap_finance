...
1. **철강의 배출 원단위는 검증되지만 석유화학의 것은 검증되지 않는다.** 철강은 시설 배출을 회사 공시 총량에 재척도하므로 모형이 쓰는 `ef_inc`가 공시와 ±1% 안에서 맞는다(NSC 1.846 vs 1.856, POSCO 1.987 vs 1.988). 루트 표준값(BF 2.15)은 **총량을 시설에 나누는 가중치**로만 쓰이고, NSC처럼 목록이 고로 단일 구성이면 가중치가 균일해져 표준값 수준이 결과에 걸리지 않는다. 반면 **석유화학은 재척도하지 않는 상향식**이라 주입값 0.95가 곧 수준이고, 두 회사 모두 생산량을 공시하지 않아 **확인할 방법이 없다**. 여기가 배출 입력의 진짜 빈 곳이다. 미쓰이 오사카 공시를 절대량으로 역산하면 공시 감축량(0.70 Mt)이
   우리가 그 크래커에 넣은 배출 총량(0.389 Mt)보다 크다(H3 §4-1). 능력 가설은 경산성
   조사로 배제됐고(455 kt 정확히 일치), 공시 감축량이 공장 전체 배출의 44%라는 점에서
   **크래커 밖(유틸리티·유도품 가열로)을 포함할 가능성이 높다**. 그래도 우리 NCC EF
   0.95가 낮을 가능성은 남고, 사실이라면 **② 237~275를 과대평가**하는 방향이다.
   에틸렌 기준·출처 있는 원단위를 확보하기 전까지 값은 고치지 않는다. 상한 검증은
