...
2. **TCaR 절대값은 사전 변동성에 의존한다** (A-17). 월별 이력 확보 전까지 TCaR의 *순위*는
   방어 가능하나 *수준*은 아니다.
3. **개수 재조달가가 실적 대비 4.2배**(A-13, H3 §1-1). 조기 전환의 벌점을 과대평가해 투자를 재투자 창으로 과도하게 몰 수 있다. `reline_cheap` 묶음은 계획 메뉴 고정이라 효과의 하한(−1%)만 보여준다 — `--replan` 확인이 남았다.
4. **석유화학 원료(나프타) 미수집** — `D1b.energy_naphtha` 0/69. 기존설비 원료가 노출이
   과소평가된다. 마진이 영업이익 기준이라 이중계상은 없다.
5. **공시 해상도** — 4개 기업×시나리오 중 일부는 강제 가능한 커밋이 없어 gap을 산출하지
   않는다. 이것은 결측이 아니라 **측정 불가 판정**이다 (A-16).
6. **계획 선택 채널의 민감도는 부분적으로만 검증됐다.** 시나리오 러너는 계획 메뉴를
