...
| ID | 가정 | 근거 | 결론 영향 | 검증 |
|---|---|---|---|---|
| A-01 | 능력 = 공표 능력 우선, 미공표는 내용적×913.3 t/m³·yr | 업계 관행 | 중 (민감도 10위) | G3 미완 |
...
2. **A-02가 최대 약점이고, 수집 시도는 막혔다.** 한국 사업장별 배출은 명세서에 법정 기재 항목으로 존재하지만 **공개 발행되지 않는다** — 공공데이터포털·KIER 분석 모두 법인 단위에서 멈춘다(`docs/data_gap_registry.md`). 다만 회사 단위 앵커는 외부와 맞는다: POSCO 2021 Scope1이 우리 77.1 MtCO₂ vs KIER 78.5 MtCO₂로 **−1.8%**. 문제는 총량이 아니라 총량을 시설에 나누는 규칙이다. 감축 단가를 154% 움직이는 파라미터가 가장 약한 증거(T5) 위에 있다 — 2위 파라미터의 3.4배다(`out/sensitivity/ranking.csv`).
