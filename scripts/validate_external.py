...
def main() -> int:
    ...
    if not h2f.empty:
        ...
        L += ["| 항목 | 우리 가정 | 공시 | 판정 |", "|---|---|---|---|",
              ...
              "오류가 아니다.", "",
              "**§4-1-a에서 (a)가 배제되고 (c)가 지지된다.** 아래는 그 이전에 공시 구조만으로 "
              "세운 논거이며 결론이 같다.", "",
              ...
              "있다 — 그러나 §4-1-a가 그 가능성을 배제한다.", ""]

    # NCC 배출원단위 — D7 `quote`에 있던 공시 수치로 직접 역산 (§4-1의 미해결 항목)
    d7 = load_input(ddir, "D7_disclosed_plan")
    L += ["### 4-1-a. NCC 배출원단위 역산 — 미해결이던 것을 공시가 푼다", "",
          "§4-1은 오사카 연료전환 공시(0.70 MtCO₂)와 우리 모형 배출(0.389 Mt)이 어긋난다는 "
          "것으로 끝났다. 답은 웹이 아니라 **우리 데이터 안**에 있었다 — D7의 `quote` 컬럼은 "
          "엔진이 읽지 않는 컬럼이라 아무도 보지 않고 있었다.", "",
          "미쓰이의 에틸렌 설비 집약 공시(`MCI_RELEASES`, D7 `timing` 행): AMEC 미즈시마를 "
          "정지하고 大阪石油化学(OPC)로 집약하면 **統合前 95.1万t → 統合後 45.5万t, "
          "CO₂ ▲50.6万t/年**. 여기서 45.5万t은 JPCA가 게재한 OPC 에틸렌 능력 455,000 t/yr과 "
          "정확히 같다 — 즉 이 수치들은 **능력**이다.", "",
          "| 계산 | 값 |", "|---|---|",
          "| 정지되는 에틸렌 능력 | 951,000 − 455,000 = **496,000 t/yr** |",
          "| 공시 CO₂ 감축 | **506,000 tCO₂/yr** |",
          "| **함의 배출원단위** | 506,000 ÷ 496,000 = **1.020 tCO₂/t-에틸렌** |",
          f"| 우리 주입값 (`NCC` ROUTE EF) | 0.95 → **{100 * (0.95 - 1.020) / 1.020:+.1f}%** |", "",
          "**우리 NCC 배출계수는 회사가 자기 설비 정지로 공시한 감축량에서 역산한 값과 7% "
          "차이다 — 범위 안이다.** 석유화학 배출 원단위가 처음으로 1차 공시에 대조됐다.", "",
          "그러면 §4-1의 0.70 Mt는 무엇인가. 같은 원단위로 OPC 크래커 총배출은 0.464 Mt인데 "
          "**연료전환만으로 0.70 Mt를 줄인다는 것은 총배출보다 크다**. 두 공시는 서로 "
          "정합하지 않으며, 정합하게 만드는 유일한 읽기는 0.70 Mt의 경계가 OPC 크래커보다 "
          "**넓다**는 것이다(미쓰이 오사카공장 전체의 가열로, 또는 이치하라 포함). "
          "§4-1의 설명 (c)가 지지되고 **(a)는 배제된다** — 우리 EF가 낮은 것이 아니다.", ""]
    ...
    L += ["## 5. 아직 못 한 대조 (없는 것을 있다고 하지 않는다)", "",
          ...
          "— 석화 결과의 가장 약한 고리.",
          "- **NCC 배출원단위 문헌 앵커는 여전히 없다** — §4-1-a는 문헌이 아니라 **기업 "
          "공시 역산**이고, 단일 관측(설비 1기 정지)이다. 독립 문헌 대역이 있으면 범위를 "
          "부여할 수 있다. 수집 시도: 스팀크래커 원단위를 "
