...
"<b>MILP 품질:</b> E2는 계획 <b>순서</b>를 정하는 대리목적함수이고 정본 비용은 E4 시뮬레이션이 낸다. 상대갭 2%·시간상한 300초, 상한에서 얻은 실행가능해도 후보로 채택하며 각 계획의 <code>solve_status</code>가 <code>out/e2/plan_index.csv</code>에 기록된다."
].map(q=>`<li>${q}</li>`).join("");
renderFac("POSCO");
</script>"""
